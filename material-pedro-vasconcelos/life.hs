...
-- garantir que uma posição fica dentro do tabuleiro
wrap :: Pos -> Pos
wrap (x,y) =  ((x-1) `mod` width + 1, 
               (y-1) `mod` height + 1)

-- número de células vizinhas duma posição
...
-- código de visualização / interação
--------------------------------------------------------

{- 
  Operações de posicionamento e escrita num terminal
  NB: as sequências de controlo ANSI *não* funcionam em Windows!
-}
